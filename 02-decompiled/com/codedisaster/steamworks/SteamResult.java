...
public enum SteamResult {
    ...
    public static SteamResult byValue(int n) {
        if (n < valuesLookupTable.length) {
            return valuesLookupTable[n];
        }
    ...
    static {
        ...
        for (SteamResult steamResult : steamResultArray) {
            n = Math.max(n, steamResult.code);
        }
        valuesLookupTable = new SteamResult[n + 1];
        SteamResult[] steamResultArray2 = steamResultArray;
        int n2 = steamResultArray2.length;
        for (int i = 0; i < n2; ++i) {
            SteamResult steamResult;
            SteamResult.valuesLookupTable[steamResult.code] = steamResult = steamResultArray2[i];
        }
    }
}
