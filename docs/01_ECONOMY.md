...
经验值 (含建造者移动/准备时间):
  提取器: 33s, 制造仪: 67s, 工厂: 33s, 机甲/坦克: 17s
```
...
```
第1次分析 (错误): CC收入 = 0
  原因: 误将 gameFramework.d.e (粒子效果类) 当作 CommandCenter
  该类有 k=20.0f 字段但从未被读取 (死字段)
  n.d(float) 以为是收入方法, 实际是反作弊蜜罐

第2次分析 (错误): CC收入 = 337.5/s
  原因: 找到了正确的 game.units.d.e 类, cy()=18.0
  但使用了 30fps 帧率, 且误以为每帧都触发收入
  公式: 4.5 × 30 × 2.5 = 337.5

第3次分析 (正确): CC收入 = 67.5/s
  交叉验证: game-lib.jar + RW-HPS 服务端 + 6个回放实测
  帧率: 60fps (GameHessData.gameFPS 服务端确认)
  显示窗口: 40帧 (所有收入单位的标准窗口)
  公式: 18 × (60/40) × 2.5 = 18 × 1.5 × 2.5 = 67.5
  
  回放验证:
    15s 游戏, CC-only 玩家: 5012 - 4000 = 1012 收入
    1012 / 15 = 67.5/s ✓ (6个不同玩家)
    30s 游戏, 1x 速度:  4805 - 4000 = 805 收入
    805 / 30 = 27.0/s ✓  (确认 1x 速度公式)
```
