...
public class JoinTask5 implements Runnable {
    public void run() {
        try {
            ...
            if (servers != null && servers.size() > 0) {
                Class<?> gCls = Class.forName("com.corrodinggames.rts.gameFramework.j.g");
                // 正确格式: get|uuid|id|false|port (用户 UI 操作暴露 — 主服务器中继)
                int tried = 0;
                for (Object s : servers) {
                    if (tried >= 8) break;
                    String b = (String) gCls.getField("b").get(s);
                    int A = gCls.getField("A").getInt(s);  // 02b j/g.java:95 铁证: A=id (中继格式第3段)
                    boolean m = gCls.getField("m").getBoolean(s);
                    int g = gCls.getField("g").getInt(s);
                    if (b == null || !b.startsWith("u_") || A == 0) continue;  // A==0 = 直连服务器, 跳过
                    String addr = "get|" + b + "|" + A + "|" + m + "|" + g;
                    System.out.println("[JOIN] 尝试 " + (tried + 1) + "/8 → " + addr);
                    try {
                ...
                if (tried >= 8) System.out.println("[JOIN] 8 个服务器全部超时");
            }
            // 5. 当前文档确认
            Object doc = root.getClass().getMethod("getCurrentDocumentPath").invoke(root);
