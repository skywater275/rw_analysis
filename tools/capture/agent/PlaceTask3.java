...
public class PlaceTask3 implements Runnable {
    public void run() {
        try {
            ...
            if (tankType == null) { System.out.println("[PLC] tank 枚举未找到"); return; }
            System.out.println("[PLC] tank 类型: " + ((Enum<?>) tankType).name());
            // 2. 玩家 (当前 bs) — 第 1 队伍
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                System.out.println("[PLC] 放置 #" + i + " (" + states[i] + ") @x=" + (baseX + i * 200.0F));
            }
            ...
            for (Object o : objs) {
                ...
                // 打印所有 tank 位置 (诊断)
                if (x > 3550 && x < 4550) {
                    System.out.println("[PLC] 对象 @x=" + x + " class=" + o.getClass().getSimpleName());
                }
                for (int i = 0; i < 4; ++i) {
                    if (Math.abs(x - (baseX + i * 200.0F)) < 5.0F) {
                        StringBuilder sb = new StringBuilder("[PLC] tank#" + i + "(" + states[i] + "):");
                        for (int j = 0; j < 7; ++j) {
            ...
            System.out.println("[PLC] 找到 " + found + "/4");
            // 5. 存档差分
            ...
            System.out.println("[PLC] 存档 placetest 完成");
        } catch (Throwable t) {
            System.out.println("[PLC] 失败:"); t.printStackTrace(System.out);
        }
    }
}
