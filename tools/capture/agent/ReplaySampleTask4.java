...
public class ReplaySampleTask4 implements Runnable {
    static int count = 0;
            ...
            if (count == 1) {
                ...
                try {
                    root0.getClass().getMethod("loadReplay", String.class).invoke(
                            root0, "Beach landing (2p) [by hxyy] [v1.15] (7 6月 2026 01.22.36).replay");
    ...
    void sample() {
        try {
            ...
            System.out.println("[RPL] 采样异常: " + t + " @" + t.getStackTrace()[0]);
        }
    }
}
