...
public class LobbyTask3 implements Runnable {
    public void run() {
        try {
            ...
            if (servers != null) {
                ...
                for (Object s : servers) {
                    try {
                        Object name = gCls.getField("d").get(s);   // 服务器名 (GameServerInfo.d String)
                        Object map = gCls.getField("b").get(s);
                        Object players = gCls.getField("c").get(s);
                        System.out.println("[LOBBY]   " + name + " | map=" + map + " | players=" + players);
                    } catch (Throwable t) {
                        System.out.println("[LOBBY]   " + s);
                    }
                    if (++i >= 12) { System.out.println("[LOBBY]   ..."); break; }
                }
            }
            // 5. 当前文档确认
