...
public class PlaceTask5 implements Runnable {
    public void run() {
        try {
            ...
            if (tankType == null) { System.out.println("[PLE] tank 枚举未找到"); return; }
            System.out.println("[PLE] tank 类型: " + ((Enum<?>) tankType).name());
            // 2. 玩家 (当前 bs) — 第 1 队伍
            ...
            System.out.println("[PLE] 放置前列表大小: " + before.size());
            float baseX = 3650.0F;
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                System.out.println("[PLE] 放置 #" + i + " (" + states[i] + ") @x=" + (baseX + i * 200.0F));
            }
            ...
            System.out.println("[PLE] 放置后列表大小: " + objs.size());
            ...
            for (Object o : objs) {
                ...
                // 打印所有 tank 位置 (诊断)
                if (x > 3550 && x < 4550) {
                    System.out.println("[PLE] 对象 @x=" + x + " class=" + o.getClass().getSimpleName());
                }
                for (int i = 0; i < 4; ++i) {
                    if (Math.abs(x - (baseX + i * 200.0F)) < 5.0F) {
                        StringBuilder sb = new StringBuilder("[PLE] tank#" + i + "(" + states[i] + "):");
                        for (int j = 0; j < 7; ++j) {
            ...
            System.out.println("[PLE] 找到 " + found + "/4");
            // 5. 存档差分
            ...
            System.out.println("[PLE] 存档 placetest 完成");
        } catch (Throwable t) {
            System.out.println("[PLE] 失败:"); t.printStackTrace(System.out);
        }
    }
}
