...
public class IndTask3 implements Runnable {
    public void run() {
        try {
            ...
            System.out.println("[IND3] 当前玩家: " + me + " (k=" + me.getClass().getField("k").get(me) + ")");
            ...
            System.out.println("[IND3] 游戏对象列表: " + shells.size());
            ...
            if (target == null) { System.out.println("[IND3] 无活单位, 终止"); return; }
            Field eo = amCls.getField("eo"), ep = amCls.getField("ep");
            System.out.println("[IND3] 目标单位 @(" + eo.getFloat(target) + "," + ep.getFloat(target) + ")");
            ...
            g.invoke(target, me);
            System.out.println("[IND3] setDiscoveredBy(当前玩家) 已调用");
            // 5. 读 dF 数组
            Field dF = amCls.getDeclaredField("dF");
            dF.setAccessible(true);
            Object[] arr = (Object[]) dF.get(target);
            System.out.println("[IND3] dF 长度: " + (arr == null ? "null" : arr.length));
            if (arr != null && arr.length > 0) {
                ...
                System.out.println("[IND3] dF[0]: a(discovered)=" + aF.getBoolean(e0) + " b(typeValue)=" + bF.getInt(e0));
            }
            // 6. 存档 (验证序列化 dF 段)
            ...
            System.out.println("[IND3] 存档 indtest 完成");
        } catch (Throwable t) {
            System.out.println("[IND3] 失败:"); t.printStackTrace(System.out);
        }
    }
}
